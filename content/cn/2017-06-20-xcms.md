...
和 [workflow4metabolomics](http://workflow4metabolomics.org/sites/workflow4metabolomics.org/files/w4e2017-EC_MSprocessing_LC-MS-2017.pdf) 的教程。     

2. 虽然 xcms 已经推出了 xcms3 版本，但里面主要参数并未改变，而且很长一段时间里 xcms 和 xcms3 会共存，所以学习 xcms 并不过时。    

3. R 软件包 [IPO](https://bioconductor.org/packages/devel/bioc/vignettes/IPO/inst/doc/IPO.html) 或许是一个不错的 xcms 参数优化工具。我说“或许”是因为我并不熟悉这个软件包，尝试了几次，都因为运行时间过（> 12h）长无果而终。
...
原始数据可以用色谱-质谱公司自带的软件或 [ProteoWizard](http://proteowizard.sourceforge.net) 转化为 xcms 支持的数据格式。
...
- `prefilter`: 初过滤。prefilter = c(3, 1000) 的意思是一个色谱峰里至少有 3 个扫描点（scan），并且它的峰值强度大于 1000，它才被认为是有意义的色谱峰，而不是噪音。这一步提高了 _centWave_ 法的速度。         
...
它对应图1的 `Match Peaks Across Samples` - 数据组间的峰匹配，同时也解释了这一步的目的。确定样品间保留时间和质荷比 有微小的差异的峰是否是同一种峰。图2直观显示了通过调整保留时间和质荷比，把样品间的相同峰对应在一起。
...
- `bw`: 用于计算色谱图峰密度的高斯平滑核的标准偏差或半值宽。单位：秒。这个概念很难理解，但可以认为它描述的是样品中同一种色谱峰保留时间之间的最大差值。这个差值可以通过原始数据来估计。方法如图3-4所示：       
